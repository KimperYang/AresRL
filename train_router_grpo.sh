...
export HF_HOME=/mnt/cache/hub
export HF_DATASETS_CACHE="${HF_HOME}/datasets"
export TRANSFORMERS_CACHE="${HF_HOME}/transformers"
export XDG_CACHE_HOME="${HF_HOME}"
# Ensure W&B (wandb-core) uses system CA bundle.
...
DATA_OUT_DIR="${SCRIPT_DIR}/data/apigen_retail_v8_parquet"
VAL_COUNT=10

if [ -f "${DATA_OUT_DIR}/train.parquet" ] || [ -f "${DATA_OUT_DIR}/val.parquet" ]; then
  echo "[train_router_grpo] Found existing parquet files in ${DATA_OUT_DIR}; skip regeneration."
else
  python "${SCRIPT_DIR}/scripts/generate_apigen_router_data.py" \
    --out_dir "${DATA_OUT_DIR}" \
    --input_path "${SCRIPT_DIR}/data/apigen_retail_v8.json" \
    --val_count "${VAL_COUNT}"
fi
python -m verl.trainer.main_ppo \
  --config-path "${SCRIPT_DIR}/config" \
  --config-name apigen_router_grpo \
